...
class FinnHubUtils:
    ...
    def get_stocks_realtime_price(
        symbol: Annotated[str, "ticker symbol like AAPL, BINANCE:BTCUSDT, OANDA:EUR_USD, COMMODITIES:GOLD"]
    ) -> dict:
        """
        Retrieve real-time price data for stocks, crypto, forex, and commodities.
        ...
        try:
            ...
            if quote and quote["c"] != 0:
                return {
                    ...
                    "timestamp": datetime.fromtimestamp(quote["t"]).isoformat(),
                }
            else:
                return {"error": f"No real-time data available for symbol: {symbol}"}
    ...
    def get_basic_financials(
        ...
        output_dict = basic_financials["metric"].copy()  # Create a copy of the metrics dictionary
        
        # Update output_dict with quarterly series data
        for metric, value_list in basic_financials["series"]["quarterly"].items():
            if value_list:  # Check if the list has items
                value = value_list[0]
                output_dict[metric] = value["v"]
        
        # If selected_columns is provided, filter the dictionary
        if selected_columns:
            filtered_dict = {k: v for k, v in output_dict.items() if k in selected_columns}
